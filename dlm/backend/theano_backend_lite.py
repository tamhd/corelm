...
def variable(value, dtype=_FLOATX, name=None):
    ...
    return theano.shared(value=value, name=name, strict=False,borrow=True) # I set borrow =true by default
...
class Function(object):
    ...
    def __call__(self, inputs):
        # The old source code use literal, which is defined in python 3x
        #assert type(inputs) in {list, tuple}
        #change it to dictionary
        assert type(inputs) in {list:1, tuple:1}
...
def conv2d(x, kernel, strides=(1, 1), border_mode='valid', dim_ordering='th',
    ...
    '''
    if dim_ordering not in {'th':1, 'tf':1}:
...
def conv3d(x, kernel, strides=(1, 1, 1),
    ...
    if border_mode not in {'same':1, 'valid':1}:
...
def arange(start=0, limit=None, step=1):
  return T.arange(start, limit, step)
